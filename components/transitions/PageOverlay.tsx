...
export default function PageOverlay({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    if (active) {
      // ENTER
      gsap.set(overlayRef.current, { y: "90%" });
  ...
  return (
    <div
      ref={overlayRef}
      className="pointer-events-none will-change-transform fixed inset-0 z-100 flex items-center justify-center bg-background"
    >
      <h1
        ref={textRef}
